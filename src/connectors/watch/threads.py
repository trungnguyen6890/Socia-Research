...
@register_connector(ConnectorType.THREADS_WATCH)
class ThreadsWatchConnector(BaseConnector):
    """Watch-only stub for Threads. No automated fetching."""

    connector_type = ConnectorType.THREADS_WATCH

    async def fetch(self, since_cursor: str | None = None) -> FetchResult:
        raise NotImplementedError(
            "Threads watch is manual-only. Add content items through the admin interface."
        )
