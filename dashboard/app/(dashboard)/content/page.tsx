...
import { fmtRelative, fmtDate, jsonSafe, CONNECTOR_COLORS, cn } from '@/lib/utils';
      ...
      {items.length === 0 ? (
        ...
        <div className="block-section divide-y divide-neutral-100">
          {items.map((item) => {
            ...
            const eng = jsonSafe<Record<string, number>>(item.engagement_snapshot, {});
                  ...
                  <div className="mt-4 ml-0 border-t border-neutral-100 pt-4">
                    ...
                    <div className="grid grid-cols-3 gap-6 text-[12px]">
                      <div>
                        ...
                        <p className="text-neutral-500"><span className="text-neutral-400">Published:</span> {fmtDate(item.publish_time)}</p>
                      ...
                      <div>
                        ...
                        {Object.keys(eng).length > 0 ? (
                          Object.entries(eng).map(([k, v]) => (
                            <p key={k} className="text-neutral-500"><span className="text-neutral-400">{k}:</span> {v.toLocaleString()}</p>
                          ))
                        ) : (
                      ...
                      <div>
                        ...
                        {tags.length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {tags.map(t => <span key={t} className="text-[10px] text-neutral-500 bg-neutral-100 px-1.5 py-0.5 rounded">{t}</span>)}
                          </div>
                        ) : (
